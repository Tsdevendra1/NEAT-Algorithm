...
class GenomeNeuralNetworkMultiClass:
    ...
    def __init__(self, genome, x_train, y_train, create_weights_bias_from_genome, activation_type, learning_rate=0.0001,
        ...
        for node in self.genome.nodes.values():
            if node.node_type == 'output':
                num_output_nodes += 1
        if self.weights_dict[len(self.weights_dict)].shape[1] != num_output_nodes:
            raise ValueError('Number of outputs in weight dict does not match number of output nodes')
