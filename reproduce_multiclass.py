...
class ReproduceMultiClass:
    ...
    def create_new_population(self, population_size, num_features, num_classes):
        population = {}
        ...
        for node_id in range(num_features + num_classes):
            if node_id < num_features:
                source_node_list.append(NodeGene(node_id=node_id, node_type='source'))
            else:
                output_node_list.append(NodeGene(node_id=node_id, node_type='output', bias=1))
        ...
        for source_node in source_node_list:
            for output_node in output_node_list:
                ...
                self.innovation_tracker[(source_node.node_id, output_node.node_id)] = self.global_innovation_number
        ...
        for source_node in source_node_list:
            for output_node in output_node_list:
                ...
                connection_list.append(ConnectionGene(input_node=source_node.node_id, output_node=output_node.node_id,
        ...
        for index in range(population_size):
            ...
            # Create the genome
            population[index] = GenomeMultiClass(connections=deep_copy_connections, nodes=deep_copy_nodes,
